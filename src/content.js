...
(() => {
    ...
    usernameInputs.forEach(input => {
      if (input.type === 'text' || !input.type) {
        ...
        const hasEmailContext = 
          ...
          document.querySelector(`label[for="${input.id}"]`)?.textContent?.toLowerCase().includes('email')
  ...
  const generateLabeledEmail = (emailAddress, hostname, domainMode = "main") => {
    ...
    icon.addEventListener('click', async (event) => {
      ...
      if (currentFocusedInput) {
        try {
          ...
          const { email: emailAddress, domainMode } = await api.storage.local.get(['email', 'domainMode'])
          const trimmedEmail = (emailAddress || '').trim()
          
          if (trimmedEmail && hostname) {
            const labeledEmail = generateLabeledEmail(trimmedEmail, hostname, domainMode || 'main')
  ...
  document.addEventListener('contextmenu', (event) => {
    ...
    // Store any input field as the target
    if (target.tagName === 'INPUT' && 
        (target.type === 'text' || target.type === 'email' || target.type === 'search' || !target.type)) {
  ...
  document.addEventListener('focusin', async (event) => {
    ...
    if (target.tagName === 'INPUT' && 
      ...
      try {
        const { showFloatingIcon: iconEnabled } = await api.storage.local.get(['showFloatingIcon'])
        if (iconEnabled !== false) { // Default to true if not set
